...
namespace BomberEngine.Core.IO
{
    ...
    public class BitBufferWriter : BitBuffer
    {
        ...
        public void Write(byte source)
        {
            ...
            BitWriter.WriteByte(source, 8, m_data, m_bitLength);
            m_bitLength += 8;
        }
        ...
        {
            Debug.CheckArgument((numberOfBits > 0 && numberOfBits <= 8), "Write(byte, numberOfBits) can only write between 1 and 8 bits");
            EnsureBufferSize(m_bitLength + numberOfBits);
            BitWriter.WriteByte(source, numberOfBits, m_data, m_bitLength);
        ...
        public void Write(byte[] source)
        {
            ...
            BitWriter.WriteBytes(source, 0, source.Length, m_data, m_bitLength);
            m_bitLength += bits;
        }
        ...
        public void Write(byte[] source, int offsetInBytes, int numberOfBytes)
        {
            ...
            EnsureBufferSize(m_bitLength + bits);
            BitWriter.WriteBytes(source, offsetInBytes, numberOfBytes, m_data, m_bitLength);
            m_bitLength += bits;
        }
        ...
        public void Write(UInt16 source)
        {
            ...
            BitWriter.WriteUInt16(source, 16, m_data, m_bitLength);
            m_bitLength += 16;
        }
		...
		public unsafe void Write(Int32 source)
		{
			...
			{
				BitWriter.WriteUInt32((UInt32)source, 32, Data, m_bitLength);
			}
			m_bitLength += 32;
		}
#else
        ...
        {
            EnsureBufferSize(m_bitLength + 32);
            BitWriter.WriteUInt32((UInt32)source, 32, m_data, m_bitLength);
            m_bitLength += 32;
        }
#endif
		...
		public unsafe void Write(UInt32 source)
		{
			...
			{
				BitWriter.WriteUInt32(source, 32, Data, m_bitLength);
			}
        ...
        {
            EnsureBufferSize(m_bitLength + 32);
            BitWriter.WriteUInt32(source, 32, m_data, m_bitLength);
            m_bitLength += 32;
        }
#endif
        ...
        {
            Debug.CheckArgument((numberOfBits > 0 && numberOfBits <= 32), "Write(uint, numberOfBits) can only write between 1 and 32 bits");
            EnsureBufferSize(m_bitLength + numberOfBits);
            BitWriter.WriteUInt32(source, numberOfBits, m_data, m_bitLength);
            m_bitLength += numberOfBits;
        }
        ...
        {
            Debug.CheckArgument((numberOfBits > 0 && numberOfBits <= 32), "Write(int, numberOfBits) can only write between 1 and 32 bits");
            EnsureBufferSize(m_bitLength + numberOfBits);
            ...
            BitWriter.WriteUInt32((uint)source, numberOfBits, m_data, m_bitLength);
        ...
        {
            EnsureBufferSize(m_bitLength + numberOfBits);
            BitWriter.WriteUInt64(source, numberOfBits, m_data, m_bitLength);
            m_bitLength += numberOfBits;
        }
        ...
        {
            Debug.CheckArgument(((value >= -1.0) && (value <= 1.0)), " WriteSignedSingle() must be passed a float in the range -1 to 1; val is " + value);
        ...
        {
            Debug.CheckArgument(((value >= 0.0) && (value <= 1.0)), " WriteUnitSingle() must be passed a float in the range 0 to 1; val is " + value);
        ...
        {
            Debug.CheckArgument(((value >= min) && (value <= max)), " WriteRangedSingle() must be passed a float in the range MIN to MAX; val is " + value);
        ...
        {
            Debug.CheckArgument(value >= min && value <= max, "Value not within min/max range!");
