...
namespace BomberEngine.Core.IO
{
    public class BitWriter
    {
        ...
        {
            Debug.CheckArgument(((numberOfBits > 0) && (numberOfBits < 9)), "Read() can only read between 1 and 8 bits");
        ...
        public static void WriteByte(byte source, int numberOfBits, byte[] destination, int destBitOffset)
        {
            ...
            Debug.CheckArgument(((numberOfBits >= 0) && (numberOfBits <= 8)), "Must write between 0 and 8 bits!");
        ...
        {
            Debug.CheckArgument(((numberOfBits > 0) && (numberOfBits <= 16)), "ReadUInt16() can only read between 1 and 16 bits");
#endif
            ushort returnValue;
            if (numberOfBits <= 8)
            {
        ...
        {
            Debug.CheckArgument(((numberOfBits > 0) && (numberOfBits <= 32)), "ReadUInt32() can only read between 1 and 32 bits");
#endif
            uint returnValue;
            if (numberOfBits <= 8)
            {
        ...
        public static void WriteUInt16(ushort source, int numberOfBits, byte[] destination, int destinationBitOffset)
        {
            ...
            Debug.CheckArgument((numberOfBits >= 0 && numberOfBits <= 16), "numberOfBits must be between 0 and 16");
#if BIGENDIAN
			// reorder bytes
            ...
            numberOfBits -= 8;
            if (numberOfBits > 0)
                BitWriter.WriteByte((byte)(source >> 8), numberOfBits, destination, destinationBitOffset + 8);
        }
